...
/** Mirrors STATUS_ORDER in src/lib/actions/department.ts — only offer
 * transitions the server will actually accept, so a user never has to hit
 * the "invalid transition" error to find out a choice wasn't allowed. */
...
export function DepartmentActionsPanel({
  ...
  const canAcknowledge = STATUS_ORDER.ACKNOWLEDGED > currentOrder;
  const canProgress = STATUS_ORDER.IN_PROGRESS > currentOrder;
  const canResolve = currentOrder >= STATUS_ORDER.ACKNOWLEDGED;
    ...
    <div className="rounded-2xl border border-border bg-white p-6">
      ...
      {status === "REOPENED" && (
        <div className="mt-3 flex items-start gap-2 rounded-xl border border-status-reopened/30 bg-status-reopened-bg px-3.5 py-2.5 text-xs text-status-reopened">
          ...
          <span>
            The citizen reported this issue isn&apos;t actually resolved.
            ...
            <form action={updateFormAction} className="mt-4 flex flex-col gap-3">
              ...
              <label className="flex flex-col gap-1.5">
                ...
                <select name="status" required className={inputClasses} defaultValue="">
                  ...
                  {canAcknowledge && <option value="ACKNOWLEDGED">Acknowledged</option>}
                  {canProgress && <option value="IN_PROGRESS">In Progress</option>}
                </select>
              </label>
              <label className="flex flex-col gap-1.5">
